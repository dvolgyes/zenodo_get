...
def _fetch_record_metadata(record_id, sandbox, access_token, timeout_val, exceptions_on_failure):
    ...
    try:
        ...
        eprint(msg)
        if exceptions_on_failure: raise ConnectionError(msg)
        else: sys.exit(1)
...
def _handle_single_file_download(
    ...
    while current_retry <= retry_limit:
        if abort_signal: return False # Check before attempting download
        try:
            ...
            if current_retry <= retry_limit:
                ...
                if not error_continues:
                    ...
                    eprint(msg)
                    if exceptions_on_failure: raise Exception(msg)
                    else: sys.exit(1)
                else:
...
@click.option("-m", "--md5", "md5_opt", is_flag=True, default=False, help="Create md5sums.txt for verification.")
@click.option("-w", "--wget", "wget_file_opt", type=str, help="Create URL list for download managers. (Files will not be downloaded.)")
...
def cli(
    ...
    if cite_opt:
        ...
        print(); print("Bibtex format:"); print(zget.__bibtex__)
        sys.exit(0)
    ...
    try:
        _zenodo_download_logic(
            actual_record_id, actual_doi_str, md5_opt, wget_file_opt,
            continue_on_error_opt, keep_opt, cont_opt, retry_opt,
            pause_opt, timeout_val_opt, outdir_opt, sandbox_opt,
            access_token_opt, glob_str_opt, exceptions_on_failure=False
        )
...
def _zenodo_download_logic(
    ...
    with cd(outdir_opt):
        ...
        if actual_doi is not None:
            doi_url = actual_doi if actual_doi.startswith("http") else "https://doi.org/" + actual_doi
            try:
        ...
        if wget_file_opt:
            output_target = sys.stdout if wget_file_opt == "-" else open(wget_file_opt, "wt")
            try:
                for f_info in files_to_download:
                    fname = f_info.get("filename") or f_info["key"]
                    # Use direct link if available, else construct
                    link = f_info.get("links", {}).get("self") or f"{download_url_base}{recordID_to_fetch}/files/{fname}"
                    output_target.write(link + "\n")
            finally:
                if wget_file_opt != "-": output_target.close()
            eprint(f"URL list written to {'stdout' if wget_file_opt == '-' else wget_file_opt}.")
        ...
        total_size_val = sum((f.get("filesize") or f.get("size")) for f in files_to_download)
